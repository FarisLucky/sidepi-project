...
class LaporanMarketing extends CI_Controller {
  ...
  public function index()
  {
      ...
      $data['marketing'] = $this->modelapp->getData('id_user,nama_lengkap,no_hp,email,COUNT(id_akses) as total','user',['id_akses'=>'4'])->result_array();
  ...
  public function detailUnit($id)
  {
      ...
      $data['menus'] = $this->rolemenu->getMenus();
      $data['img'] = getCompanyLogo();
      $data['marketing'] = $this->modelapp->getData('*','user',['id_user'=>$id])->row_array();
      $data['transaksi'] = $this->modelapp->getData('no_ppjb,nama_unit,nama_properti,tgl_transaksi,total_kesepakatan,type_bayar,status_transaksi','tbl_transaksi',['id_user'=>$id])->result_array();
